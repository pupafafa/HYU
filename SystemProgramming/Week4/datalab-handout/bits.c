 ...
 *   single-precision floating point values.r
...
unsigned floatAbsVal(unsigned uf) {
	...
	if((uf&expmask)==expmask&&(uf&fracmask)) //NaN check
		return uf;
	return uf&signmask;
}
/* 
...
int signMag2TwosComp(int x) {
	...
	int x_no_sign = x^x_sign;
	int x_twos_compl = (x_no_sign^neg_mask)+(neg_mask&1);
	return x_twos_compl;
